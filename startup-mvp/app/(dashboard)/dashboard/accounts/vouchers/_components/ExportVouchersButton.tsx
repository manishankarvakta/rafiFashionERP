...
interface ExportVouchersButtonProps {
  search?: string;
  tab?: string;
  filters?: {
    type?: string;
...
export default function ExportVouchersButton({
  search = "",
  tab = "all",
  filters,
}: ExportVouchersButtonProps) {
  ...
  const handleExport = (format: "csv" | "excel") => {
    try {
      ...
      const url = `/api/export/vouchers?${params.toString()}`;
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: `Downloading vouchers as ${format.toUpperCase()}...`,
        });
      }, 1200);
    } catch (err: any) {
      setIsExporting(false);
