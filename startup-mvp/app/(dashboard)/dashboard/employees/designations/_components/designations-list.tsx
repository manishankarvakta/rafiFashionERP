...
export default function DesignationsList({
  ...
  const handleSearch = (value: string) => {
    ...
    params.set("page", "1");
    router.push(`/dashboard/employees/designations?${params.toString()}`);
  };
  ...
  const handleDelete = async () => {
    ...
    startTransition(async () => {
      const result = await trashDesignation(deleteId);
      if (result.success) {
        setDeleteId(null);
        toast({ title: "Success", description: "Designation moved to trash" });
        router.refresh();
      } else {
  ...
  const handleRestore = async () => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateDesignationStatus([restoreId], "restore");
      if (result.success) {
        setRestoreId(null);
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(initialDesignations.map((d) => d.id)));
    } else {
  ...
  const handleBulkAction = async (action: "trash" | "active" | "inactive" | "restore") => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateDesignationStatus(ids, action);
  ...
  const allSelected = initialDesignations.length > 0 && selectedIds.size === initialDesignations.length;
  ...
  return (
    <div className="space-y-4">
            ...
            {initialDesignations.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                  {isTrash ? "No trashed designations found" : "No designations found"}
                </TableCell>
              </TableRow>
            ) : (
              initialDesignations.map((item) => {
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {!isTrash && permissions?.edit && (
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/dashboard/employees/designations?action=edit&id=${item.id}`}>
                              Edit
                            </Link>
                          </Button>
                        )}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Move Designation to Trash</AlertDialogTitle>
            <AlertDialogDescription>
              This will move the designation to trash. You can restore it later from the Trash tab.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
