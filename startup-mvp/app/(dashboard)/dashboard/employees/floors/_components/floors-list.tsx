...
export default function FloorsList({
  ...
  const handleSearch = (value: string) => {
    ...
    params.set("page", "1");
    router.push(`/dashboard/employees/floors?${params.toString()}`);
  };
  ...
  const handleDelete = async () => {
    ...
    startTransition(async () => {
      const result = await trashFloor(deleteId);
      if (result.success) {
        setDeleteId(null);
        toast({ title: "Success", description: "Floor moved to trash" });
        router.refresh();
      } else {
  ...
  const handleRestore = async () => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateFloorStatus([restoreId], "restore");
      if (result.success) {
        setRestoreId(null);
        toast({ title: "Success", description: "Floor restored successfully" });
        router.refresh();
      } else {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(initialFloors.map((d) => d.id)));
    } else {
  ...
  const handleBulkAction = async (action: "trash" | "active" | "inactive" | "restore") => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateFloorStatus(ids, action);
  ...
  const allSelected = initialFloors.length > 0 && selectedIds.size === initialFloors.length;
  ...
  return (
    <div className="space-y-4">
            ...
            {initialFloors.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                  {isTrash ? "No trashed floors found" : "No floors found"}
                </TableCell>
              </TableRow>
            ) : (
              initialFloors.map((item) => {
                ...
                return (
                  <TableRow key={item.id} className={cn(isSelected && "bg-muted/50")}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{item._count?.lines || 0} lines</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline">{item._count?.employees || 0} employees</Badge>
                    </TableCell>
                    <TableCell>
                      {item.isTrash ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {!isTrash && permissions?.edit && (
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/dashboard/employees/floors?action=edit&id=${item.id}`}>
                              Edit
                            </Link>
                          </Button>
                        )}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Move Floor to Trash</AlertDialogTitle>
            <AlertDialogDescription>
              This will move the floor to trash. You can restore it later from the Trash tab.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
