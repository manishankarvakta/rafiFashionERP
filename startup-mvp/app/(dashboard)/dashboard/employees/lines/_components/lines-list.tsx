...
export default function LinesList({
  ...
  const handleSearch = (value: string) => {
    ...
    params.set("page", "1");
    router.push(`/dashboard/employees/lines?${params.toString()}`);
  };
  ...
  const handleDelete = async () => {
    ...
    startTransition(async () => {
      const result = await trashLine(deleteId);
      if (result.success) {
        setDeleteId(null);
        toast({ title: "Success", description: "Line moved to trash" });
        router.refresh();
      } else {
  ...
  const handleRestore = async () => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateLineStatus([restoreId], "restore");
      if (result.success) {
        setRestoreId(null);
        toast({ title: "Success", description: "Line restored successfully" });
        router.refresh();
      } else {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(initialLines.map((d) => d.id)));
    } else {
  ...
  const handleBulkAction = async (action: "trash" | "active" | "inactive" | "restore") => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateLineStatus(ids, action);
  ...
  const allSelected = initialLines.length > 0 && selectedIds.size === initialLines.length;
  ...
  return (
    <div className="space-y-4">
            ...
            {initialLines.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                  {isTrash ? "No trashed lines found" : "No lines found"}
                </TableCell>
              </TableRow>
            ) : (
              initialLines.map((item) => {
                ...
                return (
                  <TableRow key={item.id} className={cn(isSelected && "bg-muted/50")}>
                    ...
                    <TableCell>
                      {item.floor ? (
                        <Badge variant="secondary">{item.floor.name}</Badge>
                      ) : (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {!isTrash && permissions?.edit && (
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/dashboard/employees/lines?action=edit&id=${item.id}`}>
                              Edit
                            </Link>
                          </Button>
                        )}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Move Line to Trash</AlertDialogTitle>
            <AlertDialogDescription>
              This will move the line to trash. You can restore it later from the Trash tab.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
