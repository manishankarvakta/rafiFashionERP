...
import { trashEmployeeType, bulkUpdateEmployeeTypeStatus } from "../_actions/employee-type.action";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface EmployeeTypesListProps {
  initialEmployeeTypes: EmployeeType[];
...
export default function EmployeeTypesList({
  ...
  const handleSearch = (value: string) => {
    ...
    params.set("page", "1");
    router.push(`/dashboard/employees/types?${params.toString()}`);
  };
  ...
  const handleDelete = async () => {
    ...
    startTransition(async () => {
      const result = await trashEmployeeType(deleteId);
      if (result.success) {
        setDeleteId(null);
        toast({ title: "Success", description: "Employee type moved to trash" });
        router.refresh();
      } else {
        toast({ title: "Error", description: result.error || "Failed to delete employee type", variant: "destructive" });
      }
    });
  };
  ...
  const handleRestore = async () => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateEmployeeTypeStatus([restoreId], "restore");
      if (result.success) {
        setRestoreId(null);
        toast({ title: "Success", description: "Employee type restored successfully" });
        router.refresh();
      } else {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(initialEmployeeTypes.map((h) => h.id)));
    } else {
  ...
  const handleBulkAction = async (action: "trash" | "active" | "inactive" | "restore") => {
    ...
    startTransition(async () => {
      const result = await bulkUpdateEmployeeTypeStatus(ids, action);
  ...
  const allSelected = initialEmployeeTypes.length > 0 && selectedIds.size === initialEmployeeTypes.length;
  ...
  return (
    <div className="space-y-4">
            ...
            {initialEmployeeTypes.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                  {isTrash ? "No trashed employee types found" : "No employee types found"}
                </TableCell>
              </TableRow>
            ) : (
              initialEmployeeTypes.map((item) => {
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {!isTrash && permissions?.edit && (
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/dashboard/employees/types?action=edit&id=${item.id}`}>
                              Edit
                            </Link>
                          </Button>
                        )}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Move Employee Type to Trash</AlertDialogTitle>
            <AlertDialogDescription>
              This will move the employee type to trash. You can restore it later from the Trash tab.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
