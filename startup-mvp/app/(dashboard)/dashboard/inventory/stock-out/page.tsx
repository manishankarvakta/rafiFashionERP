...
export default async function StockOutPage({ searchParams }: StockOutPageProps) {
  ...
  const canCreate = await hasPermission(userId, "inventory.stock-out", "create");
  ...
  const stockOuts = res.success ? res.stockOuts : [];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Record and manage stock out transactions.</p>
        </div>
        <StockOutsHeaderActions
          canCreate={canCreate}
          stockOuts={stockOuts || []}
      ...
      {setupIncomplete && (
        <Alert variant="destructive">
          <AlertDescription>
            Inventory Accounting settings are incomplete. Please configure the "Negative Adjustment / Damage Expense" account in Settings to enable recording stock outs.
          </AlertDescription>
        </Alert>
      )}
