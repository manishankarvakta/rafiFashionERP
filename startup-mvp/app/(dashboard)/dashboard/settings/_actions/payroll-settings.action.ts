...
const calculationSchema = z.object({
  ...
  enableWorkHoursThresholds: z.boolean().default(true),
  minHoursForFullDay:      z.number().min(1).max(24).default(8),
  minHoursForHalfDay:      z.number().min(0.5).max(24).default(4),
  halfDayGrossSalaryPayPercentage: z.number().min(0).max(100).default(50),
  halfDayCalculationBasis: z.enum(["GROSS"]).default("GROSS"),
